...
function setIntervalAndExecute(fn, i) {
    fn()
    return setInterval(fn, i)
}
...
class Scheduler {
    ...
    dispatchAt(event, delay) {
        return setTimeout(this.dispatch(event), delay)
    }
        ...
        return new Promise((resolve, reject) => {
            this.collection.findOneAndUpdate({
                ...
                if (shouldRunAt <= this.interval * 1000) {
                    this.dispatchAt(event, event.run_at - new Date.getTime())
                }
    ...
    _init() {
        this.collection.createIndexes([
            ...
            if (err) throw err
            this.__timer = setIntervalAndExecute(this._fetch, this.interval)
        })
    }
}
