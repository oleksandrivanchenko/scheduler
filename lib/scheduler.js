...
class Scheduler {
    constructor() {
        ...
        for (let i = 1; i <= Settings.workers; ++i) {
            ...
            worker.on('done', (event, updates) => {
                ...
                if (updates && (updates.run_in || updates.run_at || updates.recurring)) {
                    if (updates.recurring && !updates.run_in && !updates.run_at) {
                        updates.run_at = Event.getNextOccurrence(event.run_at, updates.recurring)
                    }
                    this.update(event.slug, event.key, updates).catch(this.error)
                } else {
                    event.destroy().catch(this.error)
                }
            })
    ...
    add(slug, key, props = {}) {
        ...
        if (props.run_in) {
            props.run_at = new Date(Date.now() + (props.run_in * 1000))
            delete props.run_in
        }
    ...
    update(slug, key, updates = {}) {
        ...
        if (updates.run_in) {
            updates.run_at = new Date(Date.now() + (updates.run_in * 1000))
