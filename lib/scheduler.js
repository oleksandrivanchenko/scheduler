...
class Scheduler {
    ...
    add(slug, key, props = {}) {
        ...
        if (this.v) console.log(`Creating new event ${Event.getHash(slug, key)}`)
        
        return new Event(props)
            .save()
            .then(event => {
                if (event.run_at.getTime() <= Date.now() + this.interval) {
                    if (this.v) console.log(`Queueing new event ${event.getHash()}`)
    ...
    del(slug, key) {
        ...
        if (this.v) console.log(`Deleting event ${event.getHash()}`)
     ...
     */
    get(slug, key) {
        if (this.v) console.log(`Fetching event ${Event.getHash(slug, key)}`)
    ...
    update(slug, key, updates = {}) {
        if (this.v) console.log(`Updating event ${Event.getHash(slug, key)}`)
        return new Event({ slug, key })
            .save(updates)
            .then(event => {
                if (event.run_at.getTime() <= Date.now() + this.interval) {
                    if (this.v) console.log(`Queueing updated event ${event.getHash()}`)
                    return this.dispatcher.queue(event).then(Promise.resolve(event))
                } else {
                    if (this.v) console.log(`Dequeueing updated event ${event.getHash()}`)
     ...
     */
    list(before, after) {
        if (this.v) console.log('Listing events')
     ...
     */
    _fetch(__tries = 0) {
        if (this.v) console.log('Fetching next batch of events')
        return Event
