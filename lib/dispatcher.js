...
console.log('45364356345634563456343246345634563456345')
    ...
    dispatch() {
        ...
        if (!this.redis.connected) return setTimeout(() => this.dispatch(), 1000) // Wait 1s if not connected
        this.redis
            ...
            .then(hashes => {
                if (hashes.length)
                    return this.redis
                        .multi()
                        .rpush(REDIS_JOB_QUEUE, hashes[0])
                        .zrem(REDIS_JOB_ZSET, hashes[0])
                        .execAsync()
                else 
                    throw new EmptyResultError()
            })
            .then(() => setImmediate(() => console.log(7272727272)))//this.dispatch()))
            .catch(err => {
