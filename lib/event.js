 ...
 * @returns {Promise}
 */
Event.list = ({ before, after, failed } = {}) => {
    let query = { }
...
function formatFields(obj, trim = true) {
    ...
    if (obj.failed != null) {
        ...
        result.failed = !false_boolean_values.includes(obj.failed)
    }
    if (obj.request) {
        result.request = {
            host: String(obj.request.host),
            protocol: String(obj.request.protocol),
            port: parseInt(obj.request.port, 10),
            headers: obj.request.headers,
            method: String(obj.request.method),
            path: String(obj.request.path),
