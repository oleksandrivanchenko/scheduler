...
class Event {
    ...
    recur() {
        ...
        let _run_at = new Date(this.run_at)

        for (let i in this.recurring) {
            switch (i.toLowerCase()) {
                case 'years':
                    _run_at.setFullYear(_run_at.getFullYear() + this.recurring[i])
                    break
                case 'months':
                    _run_at.setMonth(_run_at.getMonth() + this.recurring[i])
                    break
                case 'weeks':
                    _run_at.setDate(_run_at.getDate() + (this.recurring[i] * 7))
                    break
                case 'days':
                    _run_at.setDate(_run_at.getDate() + this.recurring[i])
                    break
                case 'hours':
                    _run_at.setHours(_run_at.getHours() + this.recurring[i])
                    break
                case 'minutes':
                    _run_at.setMinutes(_run_at.getMinutes() + this.recurring[i])
                    break
                case 'seconds':
                    _run_at.setSeconds(_run_at.getSeconds() + this.recurring[i])
                    break
                case 'milliseconds':
                    _run_at.setMilliseconds(_run_at.getMilliseconds() + this.recurring[i])
                    break
                default:
                    break
            }
        ...
        return this.save({ run_at: _run_at.getTime() })
    }
