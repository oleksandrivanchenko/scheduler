 ...
 *      querystring {Object} - Execution request query payload
 * }
...
class Event {
    constructor({
        ...
        let obj = {
            slug,
            key,
            request: { host, protocol, port, headers, method, path, body, querystring },
            run_at,
            recurring,
            failed,
            failed_code,
...
function formatFields(obj, trim = true) {
    ...
    if (obj.request) {
        result.request = {
            ...
            querystring: _.isObject(obj.request.body) ? obj.request.querystring : null
        }
    }
